...
internal class LzxDecoder : IDisposable
{
            ...
            if (mState.BlockRemaining == 0)
            {
                ...
                switch (mState.BlockType)
                {
                    case LzxConstants.Blocktype.Aligned:
                        ...
                        MakeDecodeTable(LzxConstants.ALIGNED_MAXSYMBOLS, LzxConstants.ALIGNED_TABLEBITS, mState.AlignedLen, mState.AlignedTable);
                    ...
                    case LzxConstants.Blocktype.Verbatim:
                        ...
                        MakeDecodeTable(LzxConstants.MAINTREE_MAXSYMBOLS, LzxConstants.MAINTREE_TABLEBITS, mState.MaintreeLen, mState.MaintreeTable);
                        ...
                        MakeDecodeTable(LzxConstants.LENGTH_MAXSYMBOLS, LzxConstants.LENGTH_TABLEBITS, mState.LengthLen, mState.LengthTable);
                        break;
            ...
            while ((thisRun = (int)mState.BlockRemaining) > 0 && togo > 0)
            {
                ...
                switch (mState.BlockType)
                {
                    case LzxConstants.Blocktype.Verbatim:
                        while (thisRun > 0)
                        {
                            mainElement = (int)ReadHuffSym(mState.MaintreeTable, mState.MaintreeLen, LzxConstants.MAINTREE_MAXSYMBOLS, LzxConstants.MAINTREE_TABLEBITS, bitbuf);
                            ...
                            else
                            {
                                ...
                                {
                                    lengthFooter = (int)ReadHuffSym(mState.LengthTable, mState.LengthLen, LzxConstants.LENGTH_MAXSYMBOLS, LzxConstants.LENGTH_TABLEBITS, bitbuf);
                                ...
                                switch (matchOffset)
                                {
                                    case > 2:
                                    {
                                        ...
                                        if (extra > 3)
                                        {
                                            ...
                                            alignedBits = (int)ReadHuffSym(mState.AlignedTable, mState.AlignedLen, LzxConstants.ALIGNED_MAXSYMBOLS, LzxConstants.ALIGNED_TABLEBITS, bitbuf);
    ...
    private void ReadLengths(byte[] lens, uint first, uint last, BitBuffer buffer)
    {
        ...
        MakeDecodeTable(LzxConstants.PRETREE_MAXSYMBOLS, LzxConstants.PRETREE_TABLEBITS, mState.PretreeLen, mState.PretreeTable);

        for (x = first; x < last;)
        {
            z = (int)ReadHuffSym(mState.PretreeTable, mState.PretreeLen, LzxConstants.PRETREE_MAXSYMBOLS, LzxConstants.PRETREE_TABLEBITS, buffer);
